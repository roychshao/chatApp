...
export const getUserChatrooms: any = createAsyncThunk(
  "chatroom/getUserChatrooms",
  async (userId: string) => {
...
export const getChatroomById: any = createAsyncThunk(
  "chatroom/getChatroomById",
  async (roomId: string) => {
    const res = await axios.get(
      `http://localhost:8080/api/chatroom/${roomId}`,
      {
        withCredentials: true,
      },
    );
...
export const createChatroom: any = createAsyncThunk(
...
export const updateChatroom: any = createAsyncThunk(
  "chatroom/updateChatroom",
  async (chatroomData: chatroom) => {
    const res = await axios.post(
      "http://localhost:8080/api/chatroom/update",
      {
        roomId: chatroomData.roomId,
        roomName: chatroomData.roomName,
        users: chatroomData.users,
...
export const deleteChatroom: any = createAsyncThunk(
  "chatroom/deleteChatroom",
  async (chatroomData: chatroom) => {
    const res = await axios.post(
      "http://localhost:8080/api/chatroom/delete",
      {
        roomId: chatroomData.roomId,
        users: chatroomData.users,
        messages: chatroomData.messages,
...
const chatroomSlice = createSlice({
  ...
  reducers: {
    appendMessage: (
      state: any,
      action: PayloadAction<AppendMessagePayload>,
    ) => {
      const { roomId, body } = action.payload;
      const currentRoomIdx = state.roomProfile.rooms.findIndex(
        (room: chatroom) => {
          return room.roomId === roomId;
        },
      );
      const originMessages = state.roomProfile.rooms[currentRoomIdx].messages;
      state.roomProfile.rooms[currentRoomIdx].messages = [
        ...originMessages,
        body,
      ];
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getChatroomById.fulfilled, (state: any, action: any) => {
        const currentRoomIdx = state.roomProfile.rooms.findIndex(
          (room: chatroom) => room.roomId === action.payload.roomId,
        );
