...
describe("Register Page Component", () => {
  ...
  test("calls handleRegister on form submit", async () => {
    ...
    await waitFor(() => {
      ...
      expect(store.getDispatch).toHaveBeenCalledWith({
        type: "SET_USER_ID",
        payload: expect.any(String),
      }); // check for the call of setUserId action
      expect(store.getDispatch).toHaveBeenCalledWith({
        type: "SET_GENDER",
        payload: "female",
      }); // check for the call of setGender action
      expect(store.getDispatch).toHaveBeenCalledWith({
        type: "NAVIGATE",
        payload: "/home",
      }); // check for the call of navigate action
    });
  });
});
